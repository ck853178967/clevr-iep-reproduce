...
def run_single_example(args, model):
  ...
  img = np.array(img.resize(img_size, resample=Image.BICUBIC))
  ...
  predicted_answer = vocab['answer_idx_to_token'][predicted_answer_idx.item()] ## debug
  ...
  if predicted_program is not None:
    ...
    for fn_idx in program:
      fn_str = vocab['program_idx_to_token'][fn_idx.item()] # debug
